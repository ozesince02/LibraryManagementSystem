...
public final class SearchByAuthorStrategy implements BookSearchStrategy {
    @Override
    public List<Book> search(Collection<Book> books, String query) {
        String q = ValidationUtils.requireNonBlank(query, "Author query").toLowerCase(Locale.ROOT);
        List<Book> matches = new ArrayList<>();
        for (Book b : books) {
            if (b.getAuthor().toLowerCase(Locale.ROOT).contains(q)) {
                matches.add(b);
            }
        }
